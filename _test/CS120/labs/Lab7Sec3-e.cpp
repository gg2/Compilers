...
int main() {
	...
	cout << "How large is the hand size? (int) ";
	cin >> hand_size;
	cout << endl;
	...
	for (i=0; i < hand_size; i++) {
		hand[i].random();
		cout << "Looking at card #" << i+1 << ", it is: ";
		hand[i].look();
		cout << "Peeking at the card: ";
		hand[i].peek();
		cout << "[flip the card over]" << endl;
		hand[i].flip();
		cout << "Looking again, the card is: ";
		hand[i].look();
		cout << "Print the card properties: " << endl;
		hand[i].print();
		cout << "* * *" << endl;
	}
